//****************************************************************************************************************************
//Program name: "Archimedes' Spiral version 3".  This program shows the invisible tip of a marker drawing a spiral curve     *
//according to the equation: r = a+bθ.  Elapsed time is updated in real-time.                                                *
...
//Program name: Archimedes' Spiral version 3
//Programming language: C#
//Date project began: 2017-April-15.
//Date of revision: 2017-Nov-10
//Date of revision: 2018-Aug-26
...
//testing this program with frame size larger than HDTV (1920x1080) the program sometimes froze, and the author of this
...
{  private int form_width;               //Horizontal width measured in pixels.
   private int form_height;              //Vertical height measured in pixels.
   ...
   private const double linear_velocity = 44.5;  //pixels per second.  These are not math units; these are graphical pixels.     *
   //                                                                                                                            *
   ...
   private const double scale_factor = 50.0;
   ...
   //private Size maxframesize = new Size(form_width,form_height);         //Not needed
   //private Size minframesize = new Size(form_width,form_height);         //Not needed
   ...
   private const double b_coefficient = System.Math.Sin(t);
   ...
   //Declare a time accumulator.  This variable sums the elapsed time that the spiral is growing.  Paused time is not
   //included in this accumulator.
   double elapsed_time_accumulator = 0.0;

   //Declare a label which will contain the elapsed_time_accumulator
   Label accumulated_time = new Label();

   //Declare a label which will show a descriptive annotation about the accumulated time.
   Label accumulated_time_annotation = new Label();
   ...
   {   //Set the initial size of this form.
       ...
       t = 0.0;
       x = (initial_radius+b_coefficient)*System.Math.Cos(t);
       y = (initial_radius+b_coefficient)*System.Math.Sin(t);
   ...
   {//Explanation: The method contain 5 statements that perform a Draw action on "pointer_to_graphic_surface".  Each one of
    ...
    bic.Width = 1;
    for(int c = 1; c < form_height/scale_factor/2; c++)
...
//Candidate location for outputting the clock accumulator variable.
       accumulated_time.Text = String.Format("{0:0000.0}",elapsed_time_accumulator);
...
//Lola says:  SetVisibleCore(false); to make window invisible
//            SetVisibleCore(true); to make window visible
//
//Alternate: this.hide();
//           this.visible=false
//           Add to paint event
